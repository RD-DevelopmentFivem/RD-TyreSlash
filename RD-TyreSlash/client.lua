			...
			if CanUseWeapon(allowedWeapons) then
				...
				if closestTire ~= nil then
					...
					if IsVehicleTyreBurst(vehicle, closestTire.tireIndex, 0) == false then
						Draw3DText(closestTire.bonePos.x, closestTire.bonePos.y, closestTire.bonePos.z, tostring("~r~[E] Slash N Dash"))
						if IsControlJustPressed(1, 38) then
							...
							if driverServer == 0 then
								...
								TriggerServerEvent("RD-TyreSlash:TargetClient", driverServer, closestTire.tireIndex)
							end
							Citizen.Wait((animDuration / 2) * 1000)
...
RegisterNetEvent("RD-TyreSlash:SlashClientTire")
AddEventHandler("RD-TyreSlash:SlashClientTire", function(tireIndex)
	TriggerEvent("chatMessage", "Arabanın alarmı çalıyor")
	local player = PlayerId()
	local plyPed = GetPlayerPed(player)
